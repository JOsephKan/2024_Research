...
def main():
    ...
    tname = "/work/b11209013/MPAS/NCRF/merged_data/tropic_data/t/tPC.nc"
    ...
    q1name = "/work/b11209013/MPAS/NCRF/merged_data/tropic_data/Q1/Q1PC.nc"
    ...
    sec = (sym_data["q1pc1"].shape[0]-96)//36+1 

    for key in sym_data.keys():
        sym_split[key] = np.stack(
                [sym_data[key][i*36:i*36+96] for i in range(sec)],
                axis=0
                )
        asy_split[key] = np.stack(
                [asy_data[key][i*36:i*36+96] for i in range(sec)],
                axis=0
                )
    ...
    sym: dict[str, np.ndarray] = {
            "pc1":{
                "TT": np.stack(
                    [ft.PowerSpectrum(sym_split["tpc1"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QQ": np.stack(
                    [ft.PowerSpectrum(sym_split["q1pc1"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QT": np.stack(
                    [ft.CrossSpectrum(sym_split["q1pc1"][i], sym_split["tpc1"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0)
                },
            "pc2":{
                "TT": np.stack(
                    [ft.PowerSpectrum(sym_split["tpc2"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QQ": np.stack(
                    [ft.PowerSpectrum(sym_split["q1pc2"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QT": np.stack(
                    [ft.CrossSpectrum(sym_split["q1pc2"][i], sym_split["tpc2"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0)
                }
            }

    asy: dict[str, np.ndarray] = {
            "pc1":{
                "TT": np.stack(
                    [ft.PowerSpectrum(asy_split["tpc1"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QQ": np.stack(
                    [ft.PowerSpectrum(asy_split["q1pc1"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QT": np.stack(
                    [ft.CrossSpectrum(asy_split["q1pc1"][i], asy_split["tpc1"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0)
                },
            "pc2":{
                "TT": np.stack(
                    [ft.PowerSpectrum(asy_split["tpc2"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QQ": np.stack(
                    [ft.PowerSpectrum(asy_split["q1pc2"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0),
                "QT": np.stack(
                    [ft.CrossSpectrum(asy_split["q1pc2"][i], sym_split["tpc2"][i]) for i in range(sec)],
                    axis=0
                    ).mean(axis=0)
                }
            }
    ...
    asy_var: dict[str, np.ndarray] = {
            "sigma":{
                "pc1": 2*asy["pc1"]["QT"].real/asy["pc1"]["TT"],
                "pc2": 2*asy["pc2"]["QT"].real/asy["pc2"]["TT"]
                },
            "phi": {
                "pc1": np.arctan(np.imag(asy["pc1"]["QT"]), np.real(asy["pc1"]["QT"])),
                "pc2": np.arctan(np.imag(asy["pc2"]["QT"]), np.real(asy["pc2"]["QT"])),
                },
            "Coh": {
                "pc1": (np.real(asy["pc1"]["QT"])**2 + np.imag(asy["pc1"]["QT"])**2) / (asy["pc1"]["QQ"] * asy["pc1"]["TT"]),
                "pc2": (np.real(asy["pc2"]["QT"])**2 + np.imag(asy["pc2"]["QT"])**2) / (asy["pc2"]["QQ"] * asy["pc2"]["TT"]),
                }
            }
    ...
    plt.suptitle("EAPE Growth Rate (NCRF)")

    fig.colorbar(cf, ax=ax, orientation="horizontal", aspect=30, shrink=0.35, label=r"$\sigma$ [1/day]")
    plt.savefig("/home/b11209013/2024_Research/MPAS_ana/NCRF/image/GrowthRate.png", dpi=500)
    plt.show()
    ...
    plt.savefig("/home/b11209013/2024_Research/MPAS_ana/NCRF/image/GrowthRate_EOF2.png", dpi=500)
    plt.show()
