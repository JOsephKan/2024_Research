...
def main():
    ...
    tname = "/work/b11209013/MPAS/SD15/merged_data/tropic_data/t/tPC.nc"
    ...
    q1name = "/work/b11209013/MPAS/SD15/merged_data/tropic_data/Q1/Q1PC.nc"
    ...
    asy: dict[str, np.ndarray] = {
            "pc1":{
                "TT": np.stack(
                    [ft.PowerSpectrum(asy_split["tpc1"][i]) for i in range(12)],
                    axis=0
                    ).mean(axis=0),
                "QQ": np.stack(
                    [ft.PowerSpectrum(asy_split["q1pc1"][i]) for i in range(12)],
                    axis=0
                    ).mean(axis=0),
                "QT": np.stack(
                    [ft.CrossSpectrum(asy_split["q1pc1"][i], asy_split["tpc1"][i]) for i in range(12)],
                    axis=0
                    ).mean(axis=0)
                },
            "pc2":{
                "TT": np.stack(
                    [ft.PowerSpectrum(asy_split["tpc2"][i]) for i in range(12)],
                    axis=0
                    ).mean(axis=0),
                "QQ": np.stack(
                    [ft.PowerSpectrum(asy_split["q1pc2"][i]) for i in range(12)],
                    axis=0
                    ).mean(axis=0),
                "QT": np.stack(
                    [ft.CrossSpectrum(asy_split["q1pc2"][i], sym_split["tpc2"][i]) for i in range(12)],
                    axis=0
                    ).mean(axis=0)
                }
            }
    ...
    plt.suptitle("EAPE Growth Rate (SD15)")

    fig.colorbar(cf, ax=ax, orientation="horizontal", aspect=30, shrink=0.35, label=r"$\sigma$ [1/day]")
    plt.savefig("/home/b11209013/2024_Research/MPAS_ana/SD15/image/GrowthRate.png", dpi=500)
    plt.show()
    ...
    plt.savefig("/home/b11209013/2024_Research/MPAS_ana/SD15/image/GrowthRate_EOF2.png", dpi=500)
    plt.show()
