...
with nc.Dataset(path + exp + '_PC.nc', "r", mmap = True) as f:
    ...
    for key in f.variables.items():
        if not key[0] in dims.keys():
            data[key[0]] = (f[key[0]][0]+f[key[0]][1]).transpose(2, 1, 0)

ltime, llat, llon = data['lw'].shape
...
sym: dict[str, np.ndarray] = {
    var: fmt.sym(data[var])
    for var in data.keys()
}

sym["qr"] = sym["lw"] + sym["sw"]
...
chunking: dict[str, np.ndarray] = {
    var: np.array([
        sym[var][i*60:i*60+120] * hann
        for i in range(5)
    ])
    for var in sym.keys()
}
...
sigma: dict[str, np.ndarray] = {
    var: Growth_Rate(chunking[var], chunking['t'])
    for var in ["lw", "sw", "q1", "cu", "qr"]
}

## 2. Compute Phase
phase: dict[str, np.ndarray] = {
    var: Phase(chunking[var], chunking['t'])
    for var in ["lw", "sw", "q1", "cu", "qr"]
}
## 2. Compute Phase
coh2: dict[str, np.ndarray] = {
    var: Coherence(chunking[var], chunking['t'])
    for var in ["lw", "sw", "q1", "cu", "qr"]
}
...
Z_trans: dict[str, np.ndarray] = {
    var: z_transform(coh2[var])
    for var in ["lw", "sw", "q1", "cu", "qr"]
}

# %% Section 4: Statistical test
criteria: dict[str, dict[str, np.ndarray]] = {
    var: Z_trans[var].mean() + 1.64*Z_trans[var].std()
    for var in ["lw", "sw", "q1", "cu", "qr"]
}

sigma_filted: dict[str, dict[str, np.ndarray]] = {
    var: np.where(Z_trans[var] >= criteria[var], sigma[var], np.nan)
    for var in ["lw", "sw", "q1", "cu", "qr"]
}
...
def plot_sigma(var):
    ...
    c1 = plt.contourf(
        ...
        np.fft.fftshift(sigma_filted[var])[fr_plot > 0],
    ...
    plt.title(f'{exp} {var} EAPE Growth Rate')
    cb1 = plt.colorbar(
        c1, aspect=30, orientation='horizontal', label=r'$\sigma$ [day$^{-1}$]'
    )
    cb1.set_ticks(np.linspace(-8, 8, 9))
    plt.savefig(f'/home/b11209013/2024_Research/MPAS/GrowthRate/image/{exp}/{exp}_{var}.png', dpi=300)

for var in sigma_filted.keys():
    plot_sigma(var)
